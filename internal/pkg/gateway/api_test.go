...
type testDef struct {
	...
	keyPolicies        []*cp.SignaturePolicyEnvelope
}
...
func TestEndorse(t *testing.T) {
	tests := []testDef{
		...
		{
			name: "SBE policy requires msp2 peer from plan despite lower block height",
			plan: endorsementPlan{
				"g1": {{endorser: localhostMock, height: 4}, {endorser: peer1Mock, height: 4}}, // msp1
				"g2": {{endorser: peer2Mock, height: 3}, {endorser: peer3Mock, height: 2}},     // msp2
				"g3": {{endorser: peer4Mock, height: 5}},                                       // msp3
			},
			layouts: []endorsementLayout{
				{"g1": 1},
					...
					Identities: []*msp.MSPPrincipal{
						{
							...
							Principal:               marshal(&msp.MSPRole{MspIdentifier: "msp2", Role: msp.MSPRole_MEMBER}, t),
						},
					},
					Rule: &cp.SignaturePolicy{
						Type: &cp.SignaturePolicy_NOutOf_{
							NOutOf: &cp.SignaturePolicy_NOutOf{
								N: 1,
								Rules: []*cp.SignaturePolicy{
									{Type: &cp.SignaturePolicy_SignedBy{SignedBy: 0}},
								},
							},
						},
					},
		...
		{
			name: "SBE policy requires expanding a plan with an extra org peer",
			plan: endorsementPlan{
				"g1": {{endorser: localhostMock, height: 4}, {endorser: peer1Mock, height: 4}}, // msp1
				"g2": {{endorser: peer2Mock, height: 3}, {endorser: peer3Mock, height: 4}},     // msp2
				"g3": {{endorser: peer4Mock, height: 5}},                                       // msp3
			...
			keyPolicies: []*cp.SignaturePolicyEnvelope{
				{ // msp1 AND msp2
					Identities: []*msp.MSPPrincipal{
						{
							PrincipalClassification: msp.MSPPrincipal_ROLE,
							Principal:               marshal(&msp.MSPRole{MspIdentifier: "msp1", Role: msp.MSPRole_MEMBER}, t),
						},
						{
							PrincipalClassification: msp.MSPPrincipal_ROLE,
							Principal:               marshal(&msp.MSPRole{MspIdentifier: "msp2", Role: msp.MSPRole_MEMBER}, t),
					...
					Rule: &cp.SignaturePolicy{
						Type: &cp.SignaturePolicy_NOutOf_{
							NOutOf: &cp.SignaturePolicy_NOutOf{
								...
								Rules: []*cp.SignaturePolicy{
									{Type: &cp.SignaturePolicy_SignedBy{SignedBy: 0}},
									{Type: &cp.SignaturePolicy_SignedBy{SignedBy: 1}},
								},
							},
						},
					},
				...
				{ // msp3
					Identities: []*msp.MSPPrincipal{
						{
							PrincipalClassification: msp.MSPPrincipal_ROLE,
							Principal:               marshal(&msp.MSPRole{MspIdentifier: "msp3", Role: msp.MSPRole_MEMBER}, t),
						},
					},
					Rule: &cp.SignaturePolicy{
						Type: &cp.SignaturePolicy_NOutOf_{
			...
			expectedEndorsers: []string{"localhost:7051", "peer3:10051", "peer4:11051"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestSubmit(t *testing.T) {
	tests := []testDef{
		...
		},
		{
			...
			postSetup: func(t *testing.T, def *preparedTest) {
				def.server.registry.endpointFactory.connectOrderer = func(_ grpc.ClientConnInterface) ab.AtomicBroadcastClient {
					abc := &mocks.ABClient{}
...
func prepareTest(t *testing.T, tt *testDef) *preparedTest {
	...
	if epDef.proposalError != nil {
		...
		localEndorser.ProcessProposalReturns(createProposalResponseWithInterest(t, localhostMock.address, localResponse, epDef.proposalResponseStatus, epDef.proposalResponseMessage, tt.interest, tt.keyPolicies...), nil)
	}
...
func createEndpointFactory(t *testing.T, definition *endpointDef, dialer dialer) *endpointFactory {
	...
	return &endpointFactory{
		...
		connectOrderer: func(_ grpc.ClientConnInterface) ab.AtomicBroadcastClient {
			abc := &mocks.ABClient{}
...
func createProposalResponseWithInterest(t *testing.T, endorser, value string, status int32, errMessage string, interest *dp.ChaincodeInterest, keyPolicies ...*cp.SignaturePolicyEnvelope) *peer.ProposalResponse {
	response := createProposalResponse(t, endorser, value, status, errMessage)
	if interest != nil {
		response.Interest = marshal(interest, t)
	}
	for _, policy := range keyPolicies {
		response.KeyPolicies = append(response.KeyPolicies, marshal(policy, t))
	}
	return response
}
